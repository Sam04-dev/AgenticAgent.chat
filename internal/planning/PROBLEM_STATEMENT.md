...
The **AGENTICAGENT.CHAT Layer** project is a verifiable risk-management agent competing in the **ERC-8004 Hackathon**. The project recently migrated from a local contract deployment ("Path A") to the official shared infrastructure ("Path B"), which uses standardised on-chain contracts at fixed Sepolia testnet addresses managed by the hackathon organisers.

This migration was implemented in the `feat/erc-8004-shared-migration` branch and submitted as **PR #43**. The PR introduced a refactored `TaskIntent` EIP-712 struct aligned with the official Path B specification and a local audit trail (`checkpoints.jsonl`) for judge review.
...
**Root cause**: The `generated_types.ts` file is auto-generated from `.specify/spec/agenticagent-chat.yaml` via `npm run generate:types`. However, the YAML spec was **never updated** to reflect the Path B `TaskIntent` struct. The spec still defined the legacy fields (`agentId: string`, `volume: float`, `maxPrice: float`), while the committed `generated_types.ts` contained the new Path B fields (`agentId: bigint`, `agentWallet`, `amountUsdScaled`, `nonce`, `maxSlippageBps`). Re-running the generator from CI produced a file matching the *old* spec, thus creating a diff.
...
| `TS2345` | `test/integration/full_loop.test.ts:102` | Test intent object used old struct shape (`agentId: string`, `volume`, `maxPrice`), incompatible with the updated `TaskIntent` interface. |
...
- The local contract (`AgentTrustLayer.sol`) retains the legacy struct, creating divergence between the on-chain test environment and the Path B submission script used for live judging.
...
1. **Update the spec** (`.specify/spec/agenticagent-chat.yaml`) to match the official Path B `TaskIntent` fields.
...
5. **Update the local `AgentTrustLayer.sol`** struct to remain consistent with the Path B definition used in tests.
