...
Our research into the [OpenServ TypeScript SDK](https://github.com/openserv-labs/sdk) reveals a "Capability-based" architecture. To ensure maximum adoption within the OpenServ ecosystem, the AGENTICAGENT.CHAT should be exposed as a **Security Capability**.
...
- **Benefit:** Our use of EIP-712 makes AGENTICAGENT.CHAT natively compatible with any agent platform that supports Ethereum wallets (like OpenServ).
...
The `SentinelClient` will accept a `riskProvider` which is a Genkit flow that satisfies a specific `inputSchema` (TaskIntent) and `outputSchema` (RiskScore).
...
- **Identity Verification:** The `AgentTrustLayer` contract uses ERC-8004 as a fallback to verify if an agent is registered in a global, trustless registry before authorizing trades.
